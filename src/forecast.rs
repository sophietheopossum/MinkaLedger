...
pub struct Snapshot {
    ...
    /// Real transactions dated AFTER as_of: recorded, but not yet in the opening balances, which
    /// stop at as_of. The projection replays them on their day so a payment typed in ahead of
    /// time is money the forecast knows about.
    pub dated: Vec<DatedTxn>,
    pub holidays: Vec<NaiveDate>,
    pub interest_rules: Vec<InterestRule>,
    pub payment_rules: Vec<PaymentRule>,
}

/// A real transaction dated after as_of, with its legs.
#[derive(Debug, Clone)]
pub struct DatedTxn {
    pub id: i64,
    pub occurred_on: NaiveDate,
    pub description: String,
    /// The recurring-payment slot it claims, if it was recorded from one: kept on the replayed
    /// occurrence so a reader that attributes money to its series (the brief's commitments)
    /// still sees the payment under the series it belongs to.
    pub series_id: Option<i64>,
    pub occurrence_on: Option<NaiveDate>,
    /// (account_id, currency, amount_minor), real legs only: a conversion's own legs sit in the
    /// conversion accounts and only make each currency balance. Nothing reads a conversion
    /// account's projected balance, and a list of upcoming payments should not show them.
    pub legs: Vec<(i64, String, Minor)>,
}

#[derive(Debug, Clone, serde::Serialize)]
pub struct Occurrence {
    /// What produced it: "series" (a recurring payment), "real" (a transaction already in the
    /// book, dated after as_of), "interest" or "payment" (a leg generated by a rule). The id
    /// spaces of the two rule tables overlap, so `series_id` alone cannot tell a rule's legs
    /// apart; this can.
    pub kind: String,
    /// The series this is an instance of. For a real transaction, the slot it claims (0 when it
    /// claims none); for a rule's leg, the rule id negated.
    pub series_id: i64,
    ...
    /// series. Carried so the occurrence list and editor can say "leg 2 of 3". `chain_id` is
    /// what groups the hops of one wave: every hop of a chain shares the slot date.
    pub chain_id: Option<i64>,
    pub chain_seq: Option<i64>,
    pub chain_len: Option<i64>,
    /// Set when this is a real transaction dated after as_of rather than a projection of a rule.
...
pub fn project<R: Recurrence>(
    ...
    for s in &snap.series {
        ...
        // Expanded from a little before as_of, because the slot and the day the money moves
        // are not the same day: a weekend rule of "after" takes a Saturday slot to Monday, so
        // a slot just before as_of can still be owed on or after it, and "before" can take a
        // slot on or after as_of back to a day the walk will never visit. A slot is kept when
        ...
        for slot in slots {
            ...
            // An amount override applies to the PRIMARY leg. The balancing leg then absorbs
            // whatever is needed for the occurrence to sum to zero -- which is the negative of
            // the primary for the usual two-leg template, and the negative of primary plus
            // every other leg for a template that splits the remainder. A template with no
            // primary leg is taken as-is.
            let amended = ov.and_then(|o| o.amount_minor);
            let leg_amount = |p: &SeriesPosting| -> Minor {
                match (amended, p.role.as_str()) {
                    (Some(a), "primary") => a,
                    _ => p.amount_minor,
                }
            };
            let others: Minor = s
                .postings
                .iter()
                .filter(|p| p.role != "balancing")
                .map(leg_amount)
                .sum();
            for p in &s.postings {
                let amount = if amended.is_some() && p.role == "balancing" { -others } else { leg_amount(p) };
    ...
    // Real transactions dated after as_of. The opening balances stop at as_of, so these are
    // replayed on their day like any projected movement -- otherwise a payment typed in ahead
    // of time is invisible to the forecast while the account list already counts it.
    for t in &snap.dated {
        if t.occurred_on > horizon {
            continue;
        }
        for (account_id, currency, amount) in &t.legs {
            occurrences.push(Occurrence {
                kind: "real".to_string(),
                series_id: t.series_id.unwrap_or(0),
                occurrence_on: t.occurrence_on.unwrap_or(t.occurred_on).to_string(),
                value_on: t.occurred_on.to_string(),
    ...
    // An occurrence whose value date fell before as_of (a "before" weekend rule at a weekend,
    // an override moved into the past) is still unclaimed, so the money is still owed: the
    // walk applies it on as_of. Its value_on stays what it is, so a list can say it is due.
    let first_day = as_of.to_string();
    let mut by_day: HashMap<String, Vec<usize>> = HashMap::new();
    for (idx, o) in occurrences.iter().enumerate() {
        let day = if o.value_on < first_day { first_day.clone() } else { o.value_on.clone() };
...
mod tests {
    ...
    /// A payment typed in ahead of time is money the forecast must know about: the opening
    /// balance stops at as_of, so the projection replays it on its day, listed as a real one.
    #[test]
    fn a_payment_dated_after_as_of_is_replayed_on_its_day() {
        let mut s = snap();
        s.dated.push(DatedTxn {
            id: 77,
            occurred_on: d("2026-09-15"),
            description: "car insurance".into(),
            series_id: None,
            occurrence_on: None,
            legs: vec![(1, "GBP".into(), -40_000), (3, "GBP".into(), 40_000)],
        });
        // and one beyond the horizon, which is not the forecast's business
        s.dated.push(DatedTxn {
            id: 78,
            occurred_on: d("2027-03-01"),
            description: "far off".into(),
            series_id: None,
            occurrence_on: None,
            legs: vec![(1, "GBP".into(), -1), (3, "GBP".into(), 1)],
        });
        let p = run(&s, "2026-08-01", "2026-10-31", &[]);
        let real: Vec<&Occurrence> = p.occurrences.iter().filter(|o| o.txn_id == Some(77)).collect();
        assert_eq!(real.len(), 2, "both legs, once");
        assert_eq!(real[0].kind, "real");
        assert_eq!(real[0].series_id, 0, "claims no slot");
        assert_eq!(real[0].value_on, "2026-09-15");
        assert!(!p.occurrences.iter().any(|o| o.txn_id == Some(78)), "beyond the horizon");
        assert_eq!(closing(&p, 1), 580_000 - 40_000, "the balance walk counts it");
        let on_the_day = p.balances.iter().find(|b| b.account_id == 1 && b.on == "2026-09-15").unwrap();
        assert_eq!(on_the_day.balance_minor, 100_000 + 250_000 - 90_000 - 90_000 - 40_000);
    }

    /// A weekend rule of "before" can take a slot on or after as_of to a day before it. The slot
    /// is still unclaimed, so the money is still owed: it keeps its true value date (so a list
    /// can say it is due) and the walk applies it on as_of.
    #[test]
    fn a_value_date_moved_before_as_of_is_still_owed_and_counted_on_as_of() {
        let mut s = snap();
        // Rent on the 1st; 1 Aug 2026 is a Saturday, and "before" says Friday 31 July.
        s.series[1].weekend_rule = "before".into();
        let p = run(&s, "2026-08-01", "2026-08-31", &[]);
        let rent: Vec<&Occurrence> = p.occurrences.iter().filter(|o| o.series_id == 11).collect();
        assert_eq!(rent.len(), 2);
        assert_eq!(rent[0].occurrence_on, "2026-08-01", "the slot is untouched");
        assert_eq!(rent[0].value_on, "2026-07-31", "the day the money moves is told truthfully");
        let on_as_of = p.balances.iter().find(|b| b.account_id == 1 && b.on == "2026-08-01").unwrap();
        assert_eq!(on_as_of.balance_minor, 100_000 - 90_000, "and it is counted on as_of");
        assert_eq!(closing(&p, 1), 100_000 - 90_000 + 250_000);
    }

    /// The mirror: a slot just BEFORE as_of whose "after" rule moves the money to a day on or
    /// after it is still to come, and a rule expanded only from as_of would never see it.
    #[test]
    fn a_slot_before_as_of_owed_after_it_is_not_lost() {
        let mut s = snap();
        // Rent on the 1st with "after"; as_of is Sunday 2 Aug 2026, so Saturday's slot is owed
        // on Monday the 3rd. A slot whose money also moved before as_of stays out: July's.
        s.series[1].weekend_rule = "after".into();
        let p = run(&s, "2026-08-02", "2026-08-31", &[]);
        let rent: Vec<&Occurrence> = p.occurrences.iter().filter(|o| o.series_id == 11).collect();
        assert_eq!(rent.len(), 2, "August's rent, both legs, and nothing from July");
        assert_eq!(rent[0].occurrence_on, "2026-08-01");
        assert_eq!(rent[0].value_on, "2026-08-03");
        assert_eq!(closing(&p, 1), 100_000 - 90_000 + 250_000);
    }

    /// An amended amount is absorbed by the balancing leg, whatever else the template holds: a
    /// three-leg template still sums to zero after the amend.
    #[test]
    fn an_amend_on_a_split_template_still_balances() {
        let mut s = snap();
        // Rent splits into rent proper and a service charge that never changes.
        s.series[1].postings.push(SeriesPosting {
            account_id: 2, currency: "GBP".into(), amount_minor: 5_000, role: "other".into(),
        });
        s.series[1].postings[0].amount_minor = -95_000; // primary: what leaves Current
        s.overrides.insert(
            (11, d("2026-09-01")),
            Override { action: "amend".into(), moved_to: None, amount_minor: Some(-100_000), description: None },
        );
        let p = run(&s, "2026-08-15", "2026-09-15", &[]);
        let sept: Vec<&Occurrence> = p.occurrences.iter().filter(|o| o.series_id == 11 && o.occurrence_on == "2026-09-01").collect();
        assert_eq!(sept.iter().map(|o| o.amount_minor).sum::<Minor>(), 0, "still balanced");
        assert_eq!(sept.iter().find(|o| o.account_id == 1).unwrap().amount_minor, -100_000);
        assert_eq!(sept.iter().find(|o| o.account_id == 3).unwrap().amount_minor, 95_000, "the balancing leg absorbed the change");
    }
...
pub mod load {
    ...
    pub fn snapshot(conn: &Connection, as_of: NaiveDate) -> rusqlite::Result<Snapshot> {
        ...
        // Real transactions dated after as_of, with their real legs, oldest first.
        let mut st = conn.prepare(
            "SELECT t.id, t.occurred_on, t.description, t.series_id, t.occurrence_on,
                    p.account_id, p.currency, p.amount_minor
               FROM txn t
               JOIN posting p ON p.txn_id = t.id
               JOIN account a ON a.id = p.account_id
              WHERE t.occurred_on > ?1 AND a.kind <> 'conversion'
              ORDER BY t.occurred_on, t.id, p.id",
        )?;
        for row in st.query_map([as_of.to_string()], |r| {
            Ok((
                r.get::<_, i64>(0)?,
                ...
                r.get::<_, Option<String>>(4)?.map(date),
                r.get::<_, i64>(5)?,
                r.get::<_, String>(6)?,
                r.get::<_, Minor>(7)?,
            ))
        })? {
            let (id, on, desc, series_id, occurrence_on, account_id, currency, amount) = row?;
            match snap.dated.last_mut() {
                Some(t) if t.id == id => t.legs.push((account_id, currency, amount)),
                _ => snap.dated.push(DatedTxn {
                    id,
                    occurred_on: on,
                    description: desc,
                    series_id,
                    occurrence_on,
                    legs: vec![(account_id, currency, amount)],
                }),
            }
        }
